...
Lastly, the absolute gap between baseline and optimized variants (\textit{seq} and \textit{soa}) widens at larger resolutions, a natural consequence of all three sharing the same $\mathcal O(n^2)$ scaling multiplied by different per-pixel constants. This confirms that the overhead of on-the-fly coordinate recomputation (\cref{code:aos_spatial}) dominates the execution time as $N$ grows, while the optimized versions (\cref{subsec:seq} \& \cref{subsec: SoA}) maintain a more favorable scaling curve by shifting the workload to vectorizable floating-point operations. 

\paragraph{speedup} Looking at \cref{fig:seq_speedup}, the speedup of the SoA implementation over the Baseline is approximately 2.8× across all resolution tiers. This flatness is due to the cancellation of the $\mathcal O(n^2)$ term, leaving only the ratio of per-pixel-pair costs $C_{baseline} / C_{v}$, where $v=seq, soa$, and $C$ captures the average time spent evaluating one $(i,j)$ pair: memory loads, distance computation, kernel comparison and conditional accumulation (see Appendix \ref{} for full breakdown). The minimal gain of the \textit{seq} ($\times1.1$) is due to making the coordinate recomputation nearly free. This is a algorithmic (constant) speedup
...
\begin{figure}
    \begin{center}
        ...
        \caption{Total CPU cycles consumed by each sequential implementation. The SoA version achieves a ~3× reduction in cycles compared to the seq (AoS) version, confirming that SIMD vectorization effectively translates into proportional performance gains. The cycle count is consistent across bandwidth settings, reinforcing the conclusion that the workload is bound by L1 cache latency and instruction retirement throughput rather than memory bandwidth. Shaded regions denote $\pm 1$ standard deviation across diverse image sets.}
...
\end{itemize}
Empirical values derived from benchmarks: $C_{\text{baseline}} \approx X$, $C_{\text{seq}} \approx Y$, $C_{\text{soa}} \approx Z$ (where $X/Y/Z$ are in ns or cycles per pixel-pair). The ratio $C_{\text{baseline}} / C_{\text{soa}} \approx 3$ confirms that SoA's SIMD vectorization processes multiple pixels per instruction, while the constant speedup across resolutions follows from the $n^2$ cancellation — the same analysis referenced in \cref{subsubsec:profiling}.
\end{document}
