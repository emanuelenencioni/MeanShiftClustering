...
Without tiling, each of the N threads reads all N pixels from global memory every iteration — $N^2$ global loads. This can lead to a giant memory bandwidth bottleneck (e.g. 68K pixels and 5 channels this reaches ~93 GB per iteration). With a tiling strategy, each block of B threads cooperatively loads one tile of B pixels from global memory into shared memory (coalesced), then all B threads consume that tile from shared memory (5 cycles latency instead of 500). Over $\lceil N / B \rceil$ tiles, each pixel is loaded from global memory exactly once per iteration, reducing traffic from $O(N^2)$ to $O(N)$ bytes. 
