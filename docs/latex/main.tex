...
At first, a runtime-selectable kernel was implemented to test each one within the benchmark. This outlined a problem: SIMD operations can only be utilized when there is no variability in this function. Since the kernel is choosen at runtime, all the benchmark resulted in worst cases possible using SoA. Given that the project doesn't have to focus on the kernel quality of the segmentation results, It was decided to hard code the flat kernel. A deeper  analysis of this problem can be found in the Appendix(\cref{fig:kernel_problem}).
...
Let's look deep into the \cref{fig:aos_soa}, what maybe we could expect is the theoretical increase in the spatial search range when $h=100$, but the execution time does not scale linearly with the search volume. The $\mathcal O(N^2)$ 5D distance computation dominates (95-98\% of cycles), and the bandwidth only controls the branch-taken rate of the accumulator (2-5\%) this because the brute force inner loop iterates over all N pixels regardless of bandwidth, computing the full 5D distance. The bottleneck is therefore the quadratic distance computation (flat kernel), confirming the parallelization strategy is correctly targeted. The \texttt{perf} hardware-counter data sheds further light on how each memory
...
\begin{table}
\centering
\caption{Execution time (shift\_ms) at 1 thread across bandwidth.}
...
\end{tabular}
\end{table}
For \texttt{omp}, bandwidth has no measurable effect . speedup is $\approx 11\times$ regardless of bandwidth (\cref{tab:bw_dependency}). Same as sequential. For \texttt{omp\_soa}, the store is more nuanced: the absolute time at 1 thread varies with bandwidth ($\approx 33$ s at bw=20 vs $54$ s at bw=1000), but the speedup ration remains similar. This suggest the bandwidth affetcs the per-pixel-pair constant C in the SoA layout but not the parallel efficiency itself.
\paragraph{SMT}
...
\paragraph{Numerical precision limitations}
...
Future work can be focused on improving numerical consistency across parallel variants, or adopting a deterministic reduction strategy for the convergence check to guarantee identical iteration counts.
...
\end{itemize}
Empirical values derived from benchmarks: $C_{\text{baseline}} \approx 1.98$, $C_{\text{seq}} \approx 1.75$, $C_{\text{soa}} \approx 0.68$ (where $X/Y/Z$ are in ns or cycles per pixel-pair). The ratio $C_{\text{baseline}} / C_{\text{soa}} \approx 2.93$ confirms that SoA's SIMD vectorization processes multiple pixels per instruction, while the constant speedup across resolutions follows from the $n^2$ cancellation — the same analysis referenced in \cref{subsubsec:profiling}.

\subsection{Scaling profiling}\label{subsec:scaling_profiling}
Profiling was conducted via \texttt{perf stat} on two images (add res here) for both \texttt{omp} and \texttt{omp\_soa} at thread count 1,2,4,8,12 and 24, with 3 runs per configuration. Measured events included cycles, instructions, L1 data cache loads and misses, and backend stalls.
