...
Lastly, the "Baseline" implementation shows a growing latency increase at higher resolutions with respect to seq. This confirms that the overhead of on-the-fly coordinate recomputation (\cref{code:aos_spatial}) dominates the execution time as $N$ grows, while the optimized versions (\cref{subsec:seq} \& \cref{subsec: SoA}) maintain a more favorable scaling curve by shifting the workload to vectorizable floating-point operations. 


\subsubsection{Some deeper analysis} \label{subsubsec:profiling}
Let's look deep into the \cref{fig:aos_soa}, what maybe we could expect is the theoretical increase in the spatial search range when $h=100$, but the execution time does not scale linearly with the search volume. The $\mathcal O(N^2)$ 5D distance computation dominates (95-98\% of cycles), and the bandwidth only controls the branch-taken rate of the accumulator (2-5\%) this becouse the brute force inner loop iterates over all N pixels regardless of bandwidth, computing the full 5D distance. The bottleneck is therefore the quadratic distance computation (flat kernel), confirming the parallelization strategy is correctly targeted. The \texttt{perf} hardware-counter data sheds further light on how each memory
